...
def player_fill_board(board, player):
    ...
    while board[position] != "":
        print(
            f"Position {position} already occupied please choose a different position"
        )
    ...
    while True:
        if symbol == computer_symbol:
            ...
            if check_game_winner(board, symbol):
                print("Computer wins")
                break
        if symbol == player1_symbol or symbol2 == player1_symbol:
...
def main():
    game_mode = int(
        input("Please choose 1 for Computer vs Player or 2 for Player vs Player: ")
    )
    if game_mode == 1:
